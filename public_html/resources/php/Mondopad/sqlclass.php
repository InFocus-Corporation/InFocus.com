...
class M
{
     ...
     * 返回值:结果集 或 结果(出错返回空字符串)
     */
    public function selectview($viewname, $where = '1', $field = "*", $fun = '')
    {
        $rarr = array();
        if (empty($fun)) {
            $sqlStr = "select $field from $viewname where $where";
			//echo $sqlStr;
            $rt = mysql_query($sqlStr, $this->link);
            while ($rt && $arr = mysql_fetch_assoc($rt)) {
                array_push($rarr, $arr);
            ...
            $sqlStr = "select $fun($field) as rt from $viewname where $where";
			//echo $sqlStr;
            $rt = mysql_query($sqlStr, $this->link);
            if ($rt) {
                $arr = mysql_fetch_assoc($rt);
    ...
    {
		try{
		  $sqlStr = "delete from $this->table where $where";//$this->table vServices
		  return mysql_query($sqlStr,$this->link);
		}
		catch (Exception $e) {  
		  echo $e;
		}
