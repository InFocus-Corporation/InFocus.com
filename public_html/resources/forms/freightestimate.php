...
if(!empty($_REQUEST['zip'])) {
	...
	$sql = 'SELECT destinationzip, `' . $_REQUEST['ship'] . '` as ship FROM infocus_internal.shipping_zones WHERE destinationzip <= ' . $_REQUEST['zip'] . ' ORDER BY destinationzip DESC LIMIT 1';
	...
	$results = mysqli_query($connection, $sql);		
	while($row = mysqli_fetch_array($results)) {
		$shippingRates["lb".$row['Lbs']] = $row['cost']*1.3;
	}
...
<body style="text-align:center;max-width:1000px;background-color:#f2f2f2;">
<form class="C10" style="width:100%;margin:20px 0 0;padding:0 60px 0 0;background:transparent;border:none;box-shadow:none;box-sizing:border-box;">
...
function runShipping(){
	var zip = $('#zip').val().trim().substr(0,3),
		service = $('#service').val().trim();
	console.log(zip, service);
...
function calculate(){
	...
	var weightper = $('#weightper').val(),
		number = $('#number').val(),
		resloc = $('#resloc').val(),
		ressig = $('#ressig').val();

	if(sArray != null && number != "" && weightper != "") {
		var value = sArray['lb' + weightper] * number;
		var total = value;
		value = value * 0.1;
		total = total+value;

		$('#freight').html(numeral(total).format('$0,0.00'));
		...
		$('#residentialsurcharge').html(numeral(value).format('$0,0.00'));
		total = total+value;
		value = 0;

		if(ressig == "Yes"){ value = 4.9; }
		$('#residentialsignature').html(numeral(value).format('$0,0.00'));
		total = total+value;
...
$(document).ready(function() {
	$( "#zip" ).change(function() { if($('#zip').val().trim()!='') { runShipping(); }});
	$( "#service" ).change(function() { if($('#service').val().trim()!='') { runShipping(); }});
