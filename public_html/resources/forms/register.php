...
<section  id="headers" style="margin-left:40px;<?php if($_GET['regdl']=="TRUE"){echo "display:none;";} ?>">
	...
	<?php
	if($_GET['euwar'] == 1 || $pageText['Processing'] == '') {
			echo '<strong>Please fill out the below fields to register your product.  Registration Processing takes 2-5 business days.</strong><br>All warranties start from the date of <u>manufacture</u> unless a proof of purchase is attached to the registration request.';
	} else {
...
<table id="invoicetable" style="margin-bottom:20px;<?php if($_GET['euwar']==1){echo "display:none;";} ?>">
	<tbody>
		<tr>
			<td><strong>Valid proof of purchase must contain:</strong></td>
			<td rowspan="5">
