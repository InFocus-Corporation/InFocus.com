...
getProductText();
//Set Canonical
echo PHP_EOL . '<link rel="canonical" href="http://' .  $_SERVER['SERVER_NAME'] . '/accessories/' . $pn . '"/>' . PHP_EOL;
//Check if logged in user in edit mode for WYSIWYG editor
if($_GET['edit']=="true"){CMSscript("/resources/overviews/$pn-$lang.src", $pn, $homedir.'/resources/overviews/', 'admin,salespublisher,saleseditor,marketingpublisher,marketingeditor,editor', ".src", "overview", "true", "SavePage","overview");}
 ?>
<script>
$(document).ready(function() {
try{$(function() {$(".langlist").menu();})}catch(e){}
;
});
</script>	</head>
	<body>
...
//Navigation tabs.  Depending on what sections have details can contain overview, specs, accessories, videos, downloads, support, (and "used with" for accessories)
	...
	echo '<li><a href="#overview" class="active">' . translate('Overview') . '</a></li>
					...
					<li id="workswithsection"><a href="#workswith">' . translate('Works With') . '</a></li>
...
';
	...
	';
				...
				$result = mysqli_query($connection,'SELECT filename, filelocation, description, lang, extension
				FROM Downloads
				WHERE relatedproducts like "%' . $pn . ';%"');
				...
				echo'
					<li><a href="#downloads">'. translate('Downloads') . '</a></li>
					</ul>
